...
axiom good (xs : inp list) :
  size xs = arity => all (mem univ) xs =>
  exists (y : out), f xs = Some y.

(* when argument to f is bad, we get None *)

axiom bad (xs : inp list) :
  size xs <> arity \/ ! (all (mem univ) xs) =>
  f xs = None.
...
op inpss_invar (inpss : inp list list) : bool =
  all is_some (map f inpss).

lemma inpss_invar_init : 
  inpss_invar init_inpss.
proof.
...
lemma inpss_invar_filter (inpss : inp list list, g : inp list -> bool) :
  inpss_invar inpss => inpss_invar (filter g inpss).
proof.
rewrite /inpss_invar.
smt(mapP mem_filter allP map_f).
qed.

lemma inpss_invar_size (inpss : inp list list) :
  inpss_invar inpss =>
...
have H := all_is_some_map_f_inpss (f inps) _.
  by rewrite map_f.
smt(good bad).
qed.

lemma inpss_invar_size_alt (inpss : inp list list, inps : inp list) :
  inpss_invar inpss => inps \in inpss =>
  size inps = arity.
proof.
move => inv.
have := inpss_invar_size inpss _.
  done.
...
op inpss_done (inpss : inp list list) : bool =
  forall (x y : out option),
  x \in map f inpss => y \in map f inpss => x = y.
...
module G(Alg : ALG, Adv : ADV) = {
  proc main() : bool * int = {
    ...
    don <- inpss_done inpss;  (* are we already done? *)
    stage <- 0;
    queries <- fset0;
    while (!don /\ !error) {
      ...
      if (0 <= i < arity /\ ! i \in queries) {
        ...
        don <- inpss_done inpss;  (* perhaps we're done now? *)
      }
      else {
...
(* application to or function with arbitrary arity *)
